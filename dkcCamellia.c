...
@author d-kingyo / Camellia algorithm by Mitsubishi Electric and NTT
@see dkcCamellia.h
*/

#define DKUTIL_C_CAMELLIA_C
...
static void Camellia_WordByte( const uint32 *x, Byte *y )
{
	int i;
	for( i=0; i<4; i++ ){
...
static void Camellia_RotBlock( const uint32 *x, const int n, uint32 *y )
{
	int r;
	r = n & 31;
	if( r ){
...
static void Camellia_Feistel( const Byte *x, const Byte *k, Byte *y )
{
	Byte t[8];
...
static void Camellia_FLlayer( Byte *x, const Byte *kl, const Byte *kr )
{
	...
	Camellia_ByteWord( x, t );
	Camellia_ByteWord( kl, u );
	Camellia_ByteWord( kr, v );
	...
	Camellia_WordByte( t, x );
}

static void Camellia_Ekeygen( const int n, const Byte *k, Byte *e )
{
	Byte t[64];
	uint32 u[20];
	int  i;
	...
	Camellia_XorBlock( t+0, t+16, t+32 );

	Camellia_Feistel( t+32, SIGMA+0, t+40 );
	Camellia_Feistel( t+40, SIGMA+8, t+32 );

	Camellia_XorBlock( t+32, t+0, t+32 );

	Camellia_Feistel( t+32, SIGMA+16, t+40 );
	Camellia_Feistel( t+40, SIGMA+24, t+32 );

	Camellia_ByteWord( t+0,  u+0 );
	Camellia_ByteWord( t+32, u+4 );

	if( n == 128 ){
		for( i=0; i<26; i+=2 ){
			Camellia_RotBlock( u+KIDX1[i+0], KSFT1[i+0], u+16 );
			Camellia_RotBlock( u+KIDX1[i+1], KSFT1[i+1], u+18 );
			Camellia_WordByte( u+16, e+i*8 );
		}
	}
	else{
		Camellia_XorBlock( t+32, t+16, t+48 );

		Camellia_Feistel( t+48, SIGMA+32, t+56 );
		Camellia_Feistel( t+56, SIGMA+40, t+48 );

		Camellia_ByteWord( t+16, u+8  );
		Camellia_ByteWord( t+48, u+12 );

		for( i=0; i<34; i+=2 ){
			Camellia_RotBlock( u+KIDX2[i+0], KSFT2[i+0], u+16 );
			Camellia_RotBlock( u+KIDX2[i+1], KSFT2[i+1], u+18 );
			Camellia_WordByte( u+16, e+(i<<3) );
		}
	}
}

static void Camellia_EncryptBlock( const int n, const Byte *p, const Byte *e, Byte *c )
{
	int i;

	Camellia_XorBlock( p, e+0, c );
	...
	if( n == 128 ){
		...
		Camellia_SwapHalf( c );
		Camellia_XorBlock( c, e+256, c );
	}
}

static void Camellia_DecryptBlock( const int n, const Byte *c, const Byte *e, Byte *p )
{
	int i;

	if( n == 128 ){
		Camellia_XorBlock( c, e+192, p );
	}
	else{
		Camellia_XorBlock( c, e+256, p );
...
DKC_CAMELLIA *WINAPI dkcAllocCamellia(
	const unsigned char *key, size_t keylen)
{
	...
	p = (DKC_CAMELLIA *)dkcAllocate(sizeof(DKC_CAMELLIA));
	if(NULL == p){
		return NULL;
	}

	p->keyBits = keyBits;
	Camellia_Ekeygen(keyBits, key, p->ekey);

	return p;
}

int WINAPI dkcFreeCamellia(DKC_CAMELLIA **p){
	if(NULL == p){
		return edk_FAILED;
	}
...
int WINAPI dkcCamelliaEncrypt(DKC_CAMELLIA *p,
	...
	const unsigned char *src, size_t srcsize)
{
	...
	if(srcsize % dkcd_CAMELLIA_BLOCK_SIZE != 0){
		return edk_ArgumentException;
	}

	for(offset = 0; offset < srcsize; offset += dkcd_CAMELLIA_BLOCK_SIZE){
		Camellia_EncryptBlock(p->keyBits, src + offset, p->ekey, dest + offset);
	}

	return edk_SUCCEEDED;
}

int WINAPI dkcCamelliaDecrypt(DKC_CAMELLIA *p,
	unsigned char *dest, size_t destsize,
	const unsigned char *src, size_t srcsize)
{
	size_t offset;
	...
	for(offset = 0; offset < srcsize; offset += dkcd_CAMELLIA_BLOCK_SIZE){
		Camellia_DecryptBlock(p->keyBits, src + offset, p->ekey, dest + offset);
	}

	return edk_SUCCEEDED;
}

void WINAPI dkcCamelliaEncryptNoDest(DKC_CAMELLIA *p,
	unsigned char *dest_and_src, size_t dest_and_srcsize)
{
	size_t offset;
	unsigned char tmp[dkcd_CAMELLIA_BLOCK_SIZE];

	if(NULL == p || NULL == dest_and_src){
		return;
	}

	for(offset = 0; offset + dkcd_CAMELLIA_BLOCK_SIZE <= dest_and_srcsize;
		offset += dkcd_CAMELLIA_BLOCK_SIZE)
	{
		Camellia_EncryptBlock(p->keyBits, dest_and_src + offset, p->ekey, tmp);
		memcpy(dest_and_src + offset, tmp, dkcd_CAMELLIA_BLOCK_SIZE);
	}
}

void WINAPI dkcCamelliaDecryptNoDest(DKC_CAMELLIA *p,
	unsigned char *dest_and_src, size_t dest_and_srcsize)
{
	size_t offset;
	unsigned char tmp[dkcd_CAMELLIA_BLOCK_SIZE];
	...
	for(offset = 0; offset + dkcd_CAMELLIA_BLOCK_SIZE <= dest_and_srcsize;
		offset += dkcd_CAMELLIA_BLOCK_SIZE)
	{
		Camellia_DecryptBlock(p->keyBits, dest_and_src + offset, p->ekey, tmp);
		memcpy(dest_and_src + offset, tmp, dkcd_CAMELLIA_BLOCK_SIZE);
	}
}
