	...
	DKC_MD5 *p2,uint8 const *buf2, uint32 len2)
{
  ...
  uint32 t = 0;
	uint32 t2 =0;
	uint8 w,e;
	w = double_md5_update_begin(p,buf,len,&t);
	e = double_md5_update_begin(p2,buf2,len2,&t2);
	dkcmNOT_ASSERT(w != e);
	if(FALSE==w){
		return;
	}

	dkcmFORCE_NOT_ASSERT(len != len2);
  dkcmNOT_ASSERT((!t) != (!t2));
	if(t && t2){
		md5_mmx_double_update(p->abcd,p2->abcd,
			(const uint32 *)p->a8,(const uint32 *)p2->a8);
  ...
  while (len >= 64) 
  {
    ...
    if ((p->flags) & edkcMD_ByteReverse)
      ...
      dkcMD_ByteReverse(p->a8, 16);
		...
		{
      dkcMD_ByteReverse(p2->a8, 16);
		}

    //MD5Transform(p->abcd, (uint32 *) p->a8);
		md5_mmx_double_update(p->abcd,p2->abcd,
			(uint32 *)p->a8,(uint32 *)p2->a8);
    buf += 64;
    len -= 64;
  ...
  memmove(p->a8, buf, len);
	memmove(p2->a8, buf2, len2);
#else
	dkcMD5Load(p,buf,len);
	dkcMD5Load(p2,buf2,len2);
#endif
}
#if 0
static DKC_INLINE void double_MD5Final( DKC_MD5 *ctx)
{
  uint32 count;
  uint8 *p;
