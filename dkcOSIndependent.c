...
#	ifdef WIN32
	...
	MB("このテキストをこのソフトの開発者に渡せば、バグ発見の近道なります。\n \
		開発者に知らせるときはこのテキストを転記してあげましょう。");
	
	/*if(IDYES==MessageBox(NULL,
		"このままこのソフトを起動しつづけると、更なるエラーが発生する可能性があります。\n終了しますか？",
		"あってはいけない エラー!!!",
		MB_YESNO))
	{
		exit(edk_FAILED);
...
)
{
	...
	if(NULL== your_realloc  /*|| NULL == *OldPtr ||
		...
		return edk_ArgumentException;//アホ引数入れるな！！
	}
	...
	)
{
	...
	for(i=0;i<div;i++){
		...
		if(NULL==ptr){
			if(OldSize > (ra_size -= tmp_ra)){//とりあえずtmp_ra分を引く。
				break;//こら望みなしだわ
			}
			continue;
		}else{
...
{
	void *p;
#if 0//stack overflow 例外が発生するのも嫌だから挙動は共通で。
	int state = 0;
	p = _alloca( size );
	if(NULL==p){
		state = 1;
	...
	*/
	while (left < right) 
	{
		...
		for ( ; ; ) {
			//while (a[i] < x) i++;
			//while(less(a[i],x)) i++;
//#error less じゃなくて greater でも、qsortと同じ仕様にするため･･･。スマン
			//while( less(get_offset(a,i * width),x) < 0){
